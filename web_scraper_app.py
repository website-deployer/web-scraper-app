...
class WebScraperApp(QMainWindow):
    ...
    def render_chat_history(self):
        # Render the chat as alternating bubbles (like ChatGPT)
        html = ""
        for role, msg, timestamp in self.chat_messages:
            if role == "user":
                html += f'''
                <div style="text-align: right; margin: 12px 0;">
                    <div style="display: inline-block; max-width: 80%; background: #2563eb; color: white; padding: 14px 18px; border-radius: 18px 18px 6px 18px; font-size: 15px; font-family: 'Segoe UI', sans-serif; box-shadow: 0 2px 8px rgba(59,130,246,0.08);">
                        {msg}
                    </div>
                    <div style="text-align: right; margin-top: 4px; font-size: 12px; color: #9ca3af;">{timestamp}</div>
                </div>
                '''
            else:  # ai
                html += f'''
                <div style="text-align: left; margin: 12px 0;">
                    <div style="display: inline-block; max-width: 80%; background: #f3f4f6; color: #1e293b; padding: 14px 18px; border-radius: 18px 18px 18px 6px; font-size: 15px; font-family: 'Segoe UI', sans-serif; box-shadow: 0 2px 8px rgba(59,130,246,0.04);">
                        {msg}
                    </div>
                    <div style="text-align: left; margin-top: 4px; font-size: 12px; color: #9ca3af;">🤖 {timestamp}</div>
                </div>
                '''
        self.ai_chat_history.setHtml(html)
    ...
    def _do_ai_response_openrouter(self, user_msg, ai_context):
        ...
        timestamp = datetime.now().strftime("%H:%M")
        self.chat_messages.append(("ai", answer.replace("\n", "<br>"), timestamp))
        self.render_chat_history()
